...
/**
 * @file
 * @brief μITRON4.0仕様標準インクルードファイル
 *
 * このファイルでは，スタンダードプロファイルで必要なものと，
 * JSPカーネル独自の拡張機能で必要なものだけを定義している．
 ...
 * @n
 * このインクルードファイルは，標準インクルードファイル（t_services.h と jsp_kernel.h）でインクルードされる．
 * また，他の ITRON仕様OS からソフトウェアをポーティングする場合などには，
 * このファイルを直接インクルードしてもよい．
...
extern ER wai_flg( ID flgid, FLGPTN waiptn, MODE wfmode, FLGPTN *p_flgptn) throw();
extern ER pol_flg( ID flgid, FLGPTN waiptn, MODE wfmode, FLGPTN *p_flgptn) throw();
extern ER twai_flg(ID flgid, FLGPTN waiptn, MODE wfmode, FLGPTN *p_flgptn, TMO tmout) throw();
 ...
 *  ビットパターンのビット数
 */
#define TBIT_TEXPTN (sizeof(TEXPTN) * CHAR_BIT) /**< タスク例外要因のビット数 */
#define TBIT_FLGPTN (sizeof(FLGPTN) * CHAR_BIT) /**< イベントフラグのビット数 */
