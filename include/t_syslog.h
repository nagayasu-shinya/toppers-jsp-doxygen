 ...
 * @brief システムログ機能
 *
 * システムログサービスは，システムのログ情報を出力するためのサービスである．
 * カーネルからのログ情報の出力にも用いるため，内部で待ち状態にはいることはない．
 * \n
 * ログ情報は，カーネル内のログバッファに書き込むか，低レベルの文字出力関数を用いて出力する．
 * どちらを使うかは，拡張サービスコールで切り換えることができる．
 * \n
 * ログバッファ領域がオーバフローした場合には，古いログ情報を消して上書きする．
 * \n
 ...
 * \n
 * このファイルをインクルードする前に，t_stddef.h と itron.h を
 * インクルードしておくことが必要である．
 *
 * @copyright 2000-2003 by Embedded and Real-Time Systems Laboratory Toyohashi Univ. of Technology, JAPAN
 * @copyright 2004      by Embedded and Real-Time Systems Laboratory Graduate School of Information Science, Na
 */
 ...
 */
extern void syslog_printf(const char *format, VP_INT *args, void (*putc)(char)) throw();
