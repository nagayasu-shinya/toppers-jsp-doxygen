 ...
 *
 * タスクが待ち状態の間は，TCB およびそこから指される WINFO を次のように設定しなければならない．
 *
 * (a)
 * TCB のタスク状態を待ち状態にする．その際に，待ち状態に伴う付属状態（TS_WAIT_????）も設定する．
 *
 * (b)
 * タイムアウトを監視するために，タイムイベントブロックを登録する．
 * 登録するタイムイベントブロックは，待ちに入るサービスコール処理関数のローカル変数として確保し，
 * それへのポインタを WINFO の tmevtb に記憶する．
 * タイムアウトの監視が必要ない場合（永久待ちの場合）には，tmevtb を NULL にする．
 *
 * 同期・通信オブジェクトに対する待ち状態の場合には，
 * 標準の WINFO に wobjcb フィールドを追加した構造体（WINFO_WOBJ，wait.h で定義）を使い，
 * タスク状態に TS_WAIT_WOBJ を設定する．
 * また，以下の (c)〜(e) の設定を行う必要がある．
 * 同期・通信オブジェクトに関係しない待ち（起床待ち，時間経過待ち）の場合には，(c)〜(e) は必要ない．
 *
 * (c)
 * TCB を待ち対象の同期・通信オブジェクトの待ちキューにつなぐ．
 * 待ちキューにつなぐために，task_queue を使う．
 * TCB を同期・通信オブジェクトの管理ブロックの共通部分（WOBJCB）の待ちキューにつないだ場合には，
 * タスク状態に TS_WAIT_WOBJCB を設定する．
 *
 * (d)
 * 待ち対象の同期・通信オブジェクトの管理ブロックへのポインタを，
 * WINFO_WOBJ の wobjcb に記憶する．
 * JSPカーネルで wobjcb を使うのは，
 * 優先度順の待ちキューにつながれているタスクの優先度が変更された場合のみであるが，
 * デバッグ情報を取り出しやすいように，待ちキューが優先度順でない場合にも wobjcb を設定する．
 *
 * (e)
 * 待ち対象の同期・通信オブジェクトに依存して記憶することが必要な情報がある場合には，
 * WINFO_WOBJ に必要な情報のためのフィールドを追加した構造体を定義し，WINFO_WOBJ の代わりに用いる．
 *
 * 待ち状態を解除する際には，待ち解除したタスクからの返値を WINFO の wercd に設定する．
 * wercd を使うのは待ち解除以降であるのに対して，tmevtb は待ち解除後は使わないため，
 * メモリ節約のために共用体（union）を使っている．
 */
 ...
 * 値が変化するためにRAMに置かなければならない部分（タスク管理ブロック，TCB）に分離し，
 * TCB内に対応するタスク初期化ブロックを指すポインタを入れる．
 * タスク初期化ブロック内に対応するTCBを指すポインタを入れる方法の方が，
 * RAMの節約の観点からは望ましいが，実行効率が悪くなるために採用していない．
 * 他のオブジェクトについても同様に扱う．
 * タスク初期化ブロックには，DEF_TEX で定義されるタスク例外処理ルーチンに関する情報も含む．
 */
 ...
 * ターゲット依存にフィールドのビット幅を変更することを許している．
 */
 ...
 * 1 に固定されているため，キューイングされているかどうかの真偽値で表現することができる．
 * また，強制待ち要求ネスト数の最大値（TMAX_SUSCNT）が 1 に固定されているので，
 * 強制待ち要求ネスト数（suscnt）は必要ない．
 */
 ...
 * また，必要な場合には最高優先順位のタスクを更新し，ディスパッチ許可状態であれば TRUE を返す．
 * そうでない場合は FALSE を返す．
 */
 ...
 * 保留例外要因をクリアし，タスク例外処理禁止状態にし，CPUロックを解除する．
 * タスク例外処理ルーチンから戻ると，まずCPUロック状態に戻し，
 * その間に保留例外要因が 0 でなくなっていれば，再びタスク例外処理ルーチンを呼び出す．
 * 保留例外要因が 0 の場合には，例外処理許可状態にして関数からリターンする．
 * この関数は，実行状態のタスクが，タスク例外処理許可状態（enatex が TRUE）で，
 * 保留例外要因が 0 でない（texptn が 0 でない）場合に呼び出すことを想定している．
 * この関数は，CPUロック状態で呼び出さなければならない．
 */
 ...
 * タスク例外処理ルーチンを呼び出す．CPU例外処理ルーチンを呼び出す時は，
 * 一時的にCPUロックを解除する．
 * この関数は，ディスパッチャや割込みハンドラ／CPU例外ハンドラの
 * 出口処理から呼び出されることを想定しているが，
 * 同等の処理をターゲット依存部で記述してもよい．
 * その場合には，OMIT_CALLTEX をマクロ定義すればよい．
 */
