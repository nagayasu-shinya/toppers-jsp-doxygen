 ...
 *
 * @par 機能
 * flgidで指定されるイベントフラグに対して，setptnで指定されるビットをセットする．
 * 具体的には，対象イベントフラグのビットパターンを，
 * サービスコール呼出し前のビットパターンと setptn の値のビット毎の論理和に更新する．
 * \n
 * 対象イベントフラグのビットパターンが更新された結果，
 * そのイベントフラグで待っているタスクの待ち解除条件を満たした場合には，
 * 該当するタスクを待ち解除する．具体的には，
 * イベントフラグの待ち行列の先頭のタスクから順に待ち解除条件を満たしているかを調べ，
 * 待ち解除条件を満たしているタスクが見つかれば，そのタスクを待ち解除する．
 * また，待ち解除されたタスクに対しては，待ち状態に入ったサービスコールの返値として E_OK を返し，
 * 待ち解除時のビ ットパターンとして，この時の(待ち解除条件を満たした)ビットパターンを返す．
 * またこの時，対象イベントフラグ属性に TA_CLR(= 0x04)が 指定されている場合には，
 * イベントフラグのビットパターンのすべてのビットをクリアし，
 * サービスコールの処理を終了する．TA_CLR が指定されていない場合には，
 * 待ち行列のさらに後ろのタスクについても待ち解除の条件を満たしているかを調べ，
 * 待ち行列の最後のタスクまで調べた時点で終了する．
 * 待ち解除条件については，wai_flg の機能説明を参照すること．
 * \n
 * イベントフラグ属性に TA_WMUL(= 0x02)が指定されており，TA_CLR が指 定されていない場合，
 * set_flg の一回の呼出しで複数のタスクが待ち解除される可能性がある．
 * この場合，待ち解除されるタスクが複数ある場合には，
 * イベントフラグの待ち行列につながれていた順序で待ち解除される．
 * そのため，実行可能状態に移行したタスクで同じ優先度を持つものの間では，
 * 待ち行列の中で前につながれていたタスクの方が高い優先順位を持つことになる．
 *
 * @par 補足説明
 * setptn の全ビットが 0 の場合には，何もしない．
 *
 * @par μITRON3.0 仕様との相違
 * setptn のデータ型を UINT から FLGPTN に変更した．
 */
