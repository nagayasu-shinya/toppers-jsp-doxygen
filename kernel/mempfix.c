 ...
 * @param[in]  tmout タイムアウト指定（tget_mpfのみ）
 * @retval E_OK    正常終了
 * @retval E_ID    不正ID番号（mpfidが不正あるいは使用できない）
 * @retval E_NOEXS オブジェクト未生成（対象固定長メモリプールが未登録）
 * @retval E_PAR   パラメータエラー（p_blk，tmoutが不正）
 * @retval E_RLWAI 待ち状態の強制解除（待ち状態の間にrel_wai を受付；pget_mpf以外）
 * @retval E_TMOUT ポーリング失敗またはタイムアウト（get_mpf以外）
 * @retval E_DLT   待ちオブジェクトの削除（待ち状態の間に対象固定長メモリプールが削除；pget_mpf以外）
 */
